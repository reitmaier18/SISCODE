<?php 
    ...
    class sistem
    {
    	...
    	function listar_log(){
    		$sql = ("select date(fecha) as fecha, extract(hour from fecha) as hora, extract(minute from fecha) as minuto, extract(second from fecha) as segundo, ip, nombre, apellido, nacionalidad,cedula, descripcion from sisco.log_sistema inner join sisco.usuario on sisco.log_sistema.usuario_id = sisco.usuario.id");
    		$query=pg_query($sql);
